(function () {'use strict';
    ...
    function getLinkFunction($http, theme, util, type) {
        return function (scope, element, attrs) {
            ...
            function getSizes(config) {
                ...
                ndWrapper.style.width = attrs.width || config.width ;
                ndWrapper.style.height = attrs.height || config.height ;
    ...
    angular.module('angular-echarts.util', []).factory('util', function () {
        ...
        function getAxisTicks(data, config, type) {
            ...
            return {
                // type: config.type || 'category', //xuxin
                type:  'category',
                ...
                if (!isAxisChart(type)) {
                    ...
                    angular.forEach(serie.datapoints, function (datapoint) {
                        conf.data.push({
                            value: datapoint.y,
                            name: datapoint.x,
                            selected:datapoint.selected||false //xuxin 新增一行
                        });
                    });
                }
                if (isPieChart(type)) {
         ...
         * get tooltip config
         */
        function getColor(data, config, type) {//xuxin 封装bar颜色设置
